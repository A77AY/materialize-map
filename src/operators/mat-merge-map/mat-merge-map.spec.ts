...
import { matMergeMap } from "./mat-merge-map";

const { marbleAssert } = rxSandbox;

describe("matMergeMap", () => {
    const SIMPLE_MAT_MERGE_MAP = matMergeMap((v) => of(v));

    it("Should only complete", () => {
        const { hot, getMessages, e } = rxSandbox.create(true);
        const actual = hot("-|").pipe(SIMPLE_MAT_MERGE_MAP);
    ...
    it("Should next", () => {
        const { hot, getMessages, e } = rxSandbox.create(true);
        const actual = hot("-a-|").pipe(SIMPLE_MAT_MERGE_MAP);
    ...
    it("Should error", () => {
        const { hot, getMessages, e } = rxSandbox.create(true);
        const actual = hot("-a-|").pipe(matMergeMap(() => throwError("e")));
    ...
    describe("Double", () => {
        it("Should double next", () => {
            const { hot, getMessages, e } = rxSandbox.create(true);
            const actual = hot("-a-b-|").pipe(SIMPLE_MAT_MERGE_MAP);
        ...
        it("Should double next with delays", () => {
            ...
            const actual = hot("-a-b...100...|").pipe(matMergeMap((v) => of(v).pipe(delay(5, scheduler))));
            const expected = e("-s-S--(nc)-(NC)...95...|", VALUES);
            marbleAssert(getMessages(actual)).to.equal(expected as any);
        });

        it("Should double next with double emitions", () => {
            const { hot, getMessages, e } = rxSandbox.create(true);
            const actual = hot("-a-|").pipe(matMergeMap((v) => of(v, "x")));
